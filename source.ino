...
void loop(){
  ...
  // Print results to Serial Monitor to 2 decimal places, for debuging reasons
  Serial.print("Voltage = ");
  Serial.println(in_voltage, 2);

  // we use dealyMs to have a buffer of time between the next handeling of input from the sensor
  if(in_voltage > 3){

    setServoPosition(Positon::CLOSED);
    delayMs(3000);

  }else if(in_voltage > 1.50){

    setServoPosition(Positon::PART_OPEN);
    delayMs(3000);

  }else{

    setServoPosition(Positon::OPEN);
    delayMs(3000);
